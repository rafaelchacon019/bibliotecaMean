...
const updateProveedor = async(req, res) => {
    ...
    return !updateProveedor ? res.status(400).send("Proveedor not found") : res.status(200).send({ updateProveedor });
};

const deleteProveedor = async(req, res) => {
    const deleteProveedor = await proveedor.findByIdAndDelete({ _id: req.params['_id'] });
    return !deleteProveedor ? res.status(400).send("Proveedor not found") : res.status(200).send("Proveedor deleted");
};
